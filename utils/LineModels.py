...
# def logSFR_Behroozi(z, logMList):
#     # Halo masses should be in unit of M_sun.
#     logMListSort = np.sort(logMList)
#     argsort = np.argsort(logMList)
    
#     Ndata = len(logMListSort)
#     Nread = 1e8
#     Nsplit = int(Ndata/Nread) + 1
#     logMSplitList = np.array_split(logMListSort, Nsplit)
    
#     # Behroozi’s “sfr_release.dat” has halo masses in unit of M_sun.
#     filename_SFR = '/mnt/home/fnikakhtar/lim/release-sfh_z0_z8_052913/sfr/sfr_release.dat'
#     df_SFR = np.loadtxt(filename_SFR)

#     zp1, log10Mh, log10SFR, log10SM = df_SFR[:, 0], df_SFR[:, 1], df_SFR[:, 2], df_SFR[:, 3]
#     numlines = np.size(df_SFR, 0) 
#     num_z = len(np.unique(zp1))
#     num_M = int(numlines/num_z)
#     logMh_min, logMh_max = log10Mh.min(), log10Mh.max()
...
#     interp_spline_SFR = interp2d(X1_logM, Y1_z, Z1_logSFR, kind='cubic')
    
#     z_max = zp1.max() - 1.
    
#     result = np.array([])
#     if z <= z_max:
#         for logM in logMSplitList:
#             result = np.append(result, interp_spline_SFR(logM, z))
#     else:
#         for logM in logMSplitList:
#             result = np.append(result, interp_spline_SFR(logM, z) + 0.2943*(z - 8))
#         alt = 3.3847 - 0.2413*z
#         result[alt < result] = alt

#     output = np.empty(logMListSort.shape)
#     output[argsort] = result
